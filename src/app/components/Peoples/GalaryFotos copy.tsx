...
export function GalaryFotos() {
  ...
  const fotoData2 = fotoData.slice(1, 5);

  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-4">
      <h2 className="text-3xl font-bold text-center mb-12">Company Memories</h2>
      {fotoData2.map((item) => {
        ...
        return (
          <figure key={item.id} className="text-center group relative">
            <img src={imgSrc} alt={item.title} className="w-full h-auto rounded-xl shadow-md object-cover" />
